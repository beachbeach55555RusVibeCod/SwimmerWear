...
function block_faq($d) {
  $items = [
    ['q'=>'Можно ли носить парку зимой? До какой температуры она рассчитана?', 'a'=>'Парка рассчитана на холодную, ветреную и влажную погоду. Комфортная температура зависит от активности и слоёв одежды под паркой: в мороз рекомендуем использовать тёплый базовый и утепляющий слой.'],
    ['q'=>'Она полностью непромокаемая или водоотталкивающая?', 'a'=>'Мембрана 10K / 10K защищает от дождя и мокрого снега, а DWR-пропитка помогает воде скатываться с внешней ткани. Проклеенные швы дополнительно защищают уязвимые места.'],
    ['q'=>'Что надевать под неё?', 'a'=>'В прохладную погоду достаточно базового слоя или флиса. В более холодную — термобельё и утепляющий слой. Свободный крой позволяет менять количество слоёв по погоде.'],
    ['q'=>'Можно ли стирать в машинке?', 'a'=>'Да, на деликатном режиме при невысокой температуре. Используйте средство для мембранных тканей, не применяйте отбеливатель и обычный кондиционер для белья.'],
    ['q'=>'Нужно ли ухаживать за мембраной?', 'a'=>'Да. После нескольких стирок или когда вода перестаёт собираться каплями, водоотталкивающую обработку можно обновить специальным средством для мембранной одежды.'],
    ['q'=>'Сколько длится доставка?', 'a'=>'Срок зависит от города и выбранной службы доставки. Точный срок и стоимость показываются при подтверждении заказа.'],
  ]; ?>
<section id="faq">
  <div class="wrap">
    <div class="sec-head"><h2>Часто задаваемые вопросы</h2></div>
    <div class="faq">
...
function block_text($d) {
  if (trim((string)($d['title'] ?? '')) === 'О парке') return;
  ?>
<section>
  <div class="wrap">
