(function(){
  ...
  function init(){
    ...
    if(!document.getElementById('photo-color-ajax-style')){
      ...
      style.textContent='.photo-color-form{position:relative}.photo-color-status{display:inline-block;margin-top:5px;font-size:11px;color:#575E43;min-height:14px}.photo-color-status.is-error,.photo-color-bulk__status.is-error{color:#8B2F2F}.photo-color-bulk{display:grid;grid-template-columns:minmax(220px,1fr) minmax(170px,240px) auto minmax(120px,1fr);gap:10px;align-items:end;padding:14px 16px;margin:0 0 18px;background:#f3f4f4;border:1px solid #dfe3e3}.photo-color-bulk__label{font-size:12px;font-weight:600;align-self:center}.photo-color-bulk__status{font-size:11px;color:#575E43;align-self:center}@media(max-width:760px){.photo-color-bulk{grid-template-columns:1fr}.photo-color-bulk .btn{width:100%}}';
